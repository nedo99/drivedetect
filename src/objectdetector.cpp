...
void ObjectDetector::preprocess(const Mat& frame)
{
    ...
    blobFromImage(frame, blob, 1.0, inpSize, Scalar(), cfg->swapRb, false, CV_8U);

    // Run a model.
    net.setInput(blob, "", cfg->scale, cfg->mean);
...
vector<Rect> ObjectDetector::postprocess(const Mat& frame, const vector<Mat>& outs)
{
    ...
    NMSBoxes(boxes, confidences, cfg->confThreshold, cfg->nmsThreshold, indices);
    return boxes;
}
